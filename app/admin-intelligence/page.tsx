...
export default function MasterIntelligence() {
  ...
  const downloadGlobalReport = () => {
    ...
    const rows = assets.map(a => [
      a.schoolName, a.tailNumber, a.aircraftModel, a.tyrePosition, 
      a.serialNumber, a.currentLandings, a.maxDesignLife,
      calculateHealth(a.currentLandings, a.maxDesignLife).toFixed(0)
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers, ...rows].map(e => e.join(",")).join("\n");
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Jedo_Global_Audit.csv`);
    link.click();
  }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {Array.from(new Set(assets.map(a => a.schoolName))).map(school => (
              <div key={school} onClick={() => setSelectedSchool(school)} 
                ...
                <div>
                  <strong style={{ fontSize: '0.9rem' }}>{school}</strong>
                  <p style={{ margin: 0, fontSize: '0.65rem', color: '#64748b' }}>{assets.filter(a => a.schoolName === school).length} Assets</p>
                </div>
                <span>→</span>
              </div>
            ))}

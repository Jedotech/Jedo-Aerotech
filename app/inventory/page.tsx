...
interface InventoryPart {
  ...
  partNumber: string;
  description: string;
  priceUSD?: number; // Primary source in USD
  stockStatus?: string;
  qtyAvailable: number; 
  condition?: string;
  location?: string;
...
export default function InventoryPage() {
  ...
  return (
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* 1. NAVIGATION WITH CURRENCY SWITCHER */}
      <nav style={navStyle}>
      ...
      <header style={headerStyle}>
        <div style={{ flex: 1 }}>
            ...
            <p style={{ color: '#64748b', fontSize: '0.75rem' }}>Rate: $1 = {exchangeRate.toFixed(2)} INR</p>
        </div>
        <input 
          type="text" 
              ...
              <tbody>
                {filteredItems.map((item) => (
                  <tr key={item._id} style={trStyle}>
                    ...
                    <td style={tdStyle}>
                      <Link href={`https://jedo-fleet-intel.vercel.app/studio/intent/edit/id=${item._id}`} target="_blank" style={editLink}>EDIT</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <footer style={footerStyle}>© 2026 Jedo Technologies Pvt. Ltd. | DGCA, FAA & EASA Compliance</footer>
    </div>
  )
}
...
const activePillBtn = { backgroundColor: '#ffb400', color: '#001a35', border: 'none', padding: '4px 10px', borderRadius: '2px', fontWeight: 'bold', fontSize: '0.65rem', cursor: 'pointer' };
const inactivePillBtn = { backgroundColor: 'transparent', color: 'white', border: 'none', padding: '4px 10px', borderRadius: '2px', fontSize: '0.65rem', cursor: 'pointer' };
...
const footerStyle = { textAlign: 'center' as const, padding: '20px', fontSize: '0.7rem', color: '#94a3b8' };
    ...
    <div style={manifestPageStyle}>
      ...
      <div style={headerFlex}>
        ...
        <div style={consigneeBox}>
          ...
          <textarea value={consignee} onChange={(e) => setConsignee(e.target.value)} style={consigneeInput} rows={3} />
        </div>
      </div>
      ...
      <div style={footerFlex}>
        <div style={signatureBox}>
          ...
          <p style={{ fontSize: '0.65rem', color: '#64748b' }}>Date: {new Date().toLocaleDateString()}</p>
        </div>
      ...
      <button onClick={() => window.print()} className="no-print" style={printBtn}>PRINT MANIFEST</button>
    </div>
  )
}
...
const consigneeInput = { width: '100%', padding: '10px', fontSize: '0.9rem', border: '1px dashed #cbd5e1', borderRadius: '6px', textAlign: 'right' as any } as const;
const label = { fontSize: '0.65rem', fontWeight: '900', color: '#94a3b8', margin: '0 0 8px' } as const;
const manifestTable = { width: '100%', borderCollapse: 'collapse' as const, marginTop: '40px' } as const;
const th = { padding: '12px', textAlign: 'left' as const, fontSize: '0.7rem', color: '#64748b', borderBottom: '2px solid #001a35' } as const;
...
const printBtn = { marginTop: '60px', backgroundColor: '#001a35', color: '#fff', padding: '14px 32px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' as any } as const;
...
export default function FleetHealth() {
  ...
  const generateWaMessage = (tail: string, tyres: FleetAsset[]) => {
    ...
    const sortedTyres = [...tyres].sort((a, b) => (a.maxDesignLife - a.currentLandings) - (b.maxDesignLife - b.currentLandings));
    ...
    sortedTyres.forEach(t => {
      ...
      msg += `${statusLabel}\nPos: [${pos}] | S/N: ${t.serialNumber || 'TBD'}\nModel: ${t.tyreModel || 'Standard'}\nCond: ${t.retreadStatus || 'New'}\nRem: ${remaining} Landings\n\n`;
    });
      ...
      <nav style={navBarStyle}>
        ...
        <div style={navTitleSection}>
          <h1 style={responsiveMainTitle}>
            <span style={{ color: '#06b6d4', textShadow: '0 0 12px rgba(6, 182, 212, 0.4)' }}>{orgName?.toUpperCase() || 'OPERATOR'}</span> 
            <span style={{ color: '#475569', margin: '0 15px', fontWeight: '300' }}>/</span> 
            <span style={{ color: '#f8fafc', opacity: 0.9 }}>FLEET COMMAND</span>
          </h1>
          <p style={telemetryText}><span style={pulseDot}></span> LIVE RATE: 1 USD = ₹{exchangeRate.toFixed(2)} | SYNC: {lastSync}</p>
        </div>
        ...
        <div style={summaryGrid}>
          <div style={summaryCard}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginTop: '10px' }}>
                ...
                <div style={progressBase}><div style={{ height: '100%', width: `${avgFleetHealth}%`, backgroundColor: getHealthColor(avgFleetHealth), borderRadius: '10px' }} /></div>
            </div>
          </div>
            ...
            <div key={tail} style={aircraftCard}>
              ...
              <div style={tyreContainer}>
                {data.tyres.map((tyre) => {
                  ...
                  const cpl = (priceINR > 0 && tyre.currentLandings > 0) ? (priceINR / tyre.currentLandings).toFixed(2) : '0.00';
                  const posCodeMap: Record<string, string> = { 'Nose Gear': 'N', 'Main Left': 'ML', 'Main Right': 'MR' };
                  return (
                    <div key={tyre._id} style={{ ...technicalRow, borderLeft: `2px solid ${statusColor}` }}>
                      <div style={posCodeBox}><span style={{ fontSize: '0.7rem', fontWeight: '900', color: statusColor }}>{posCodeMap[tyre.tyrePosition || ''] || '??'}</span></div>
                      <div style={{ flexGrow: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                            <span style={makeLabel}>{tyre.manufacturer || 'Unknown'} {tyre.tyreModel && `- ${tyre.tyreModel}`}</span>
                            <span style={pnLabel}>S/N: {tyre.serialNumber || 'TBD'}</span>
                        </div>
                        <div style={assetProgressWrapper}><div style={{ height: '100%', width: `${health}%`, backgroundColor: statusColor, borderRadius: '10px' }} /></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px' }}>
                          <p style={{ margin: 0, fontSize: '0.5rem', color: '#64748b' }}>P/N: {tyre.partNumber || 'TBD'}</p>
                          <span style={{ fontSize: '0.5rem', color: '#06b6d4', fontWeight: 'bold' }}>CPL: ₹{cpl}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <Link href={`https://wa.me/919600038089?text=${generateWaMessage(tail, data.tyres)}`} target="_blank" style={orderBtn}>ORDER SPARES FOR {tail}</Link>
            </div>
          ))}
        </div>
      </main>
